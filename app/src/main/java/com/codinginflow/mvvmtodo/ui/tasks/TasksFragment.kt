...
@AndroidEntryPoint
class TasksFragment : Fragment(R.layout.fragment_task_list),TasksAdapter.OnItemClickListener {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
                ...
                when(event){
                    is TasksViewModel.TasksEvent.ShowUndoDeleteTaskMessage -> {
                        Snackbar.make(view,"Task deleted",Snackbar.LENGTH_LONG)
                            .setAction("UNDO"){
                                viewModel.onTaskUndoClicked(event.task)
                            }.show()
                    }
        ...
        setFragmentResultListener("add_edit_result"){ _, bundle ->
            val res = bundle.getInt("add_edit_result")
            viewModel.onAddEditResult(res)
        }
